    ...
    return function(){
      ...
      jail_code = function(engine, code, argv, files, stdin){
        return co(function*(){
          ...
          command = command + ("aa-exec -p " + sandboxDir + "/profile.aa " + commands[engine].cmdline_start + " " + sandboxDir + "/jailed.code");
          output = yield cmd(command);
          yield cmd("rm -rf " + sandboxDir);
          return output;
        });
      };
      configureAll = function(user){
        var profiles, name, value, results$ = [];
        profiles = require('./profiles');
        for (name in profiles) {
          value = profiles[name];
          results$.push(configure(name, value.path, value.aa, user));
        }
        return results$;
      };
