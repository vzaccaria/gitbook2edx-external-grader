...
(function(){
  ...
  _module = function(_, moment, fs, $, __, co, debug, uid, os){
    return function(){
      var cmd, writeAsync, tempDir, ref$, deployProfile, runProfile, removeProfile, default_limits, buildSandbox, writeAaProfile, writeScript, buildRunCommand, runScript, setLimits, jail_code, simplyRun, iface;
      cmd = function(it){
      ...
      buildSandbox = function*(files){
        var sandboxDir, configOptions;
      ...
      writeAaProfile = function*(profile, sandboxDir, configOptions){
        var aaProfileContent;
      ...
      writeScript = function*(profile, sandboxDir, code){
        code = ("#!" + profile.path + "\n") + code;
        yield writeAsync(sandboxDir + "/jailed.code", code, 'utf-8');
        yield cmd("chmod +x " + sandboxDir + "/jailed.code");
      };
      buildRunCommand = function*(profile, sandboxDir, configOptions){
        var command;
        command = "SANDBOXDIR=" + sandboxDir + " ";
        if (profile.env != null) {
          command = command + (" " + profile.env(configOptions) + " ");
        }
        command = command + runProfile(profile.user, sandboxDir + "/profile.aa", sandboxDir + "/jailed.code");
        debug(command);
        return command;
      };
      runScript = function*(command){
        var result, success, e;
        try {
      ...
      jail_code = function(engine, code, argv, files, stdin){
        return co(function*(){
          var profile, ref$, sandboxDir, configOptions, command, success, result;
          profile = require('./profiles')[engine];
          if (profile == null) {
            throw "Sorry, non existing profile";
          }
          ref$ = yield buildSandbox(files), sandboxDir = ref$.sandboxDir, configOptions = ref$.configOptions;
          yield writeAaProfile(profile, sandboxDir, configOptions);
          yield writeScript(profile, sandboxDir, code);
          command = yield buildRunCommand(profile, sandboxDir, configOptions);
          ref$ = yield runScript(command), success = ref$.success, result = ref$.result;
