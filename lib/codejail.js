...
(function(){
  ...
  _module = function(_, moment, fs, $, __, co, debug, uid, os){
    return function(){
      var cmd, writeAsync, commands, tempDir, ref$, deployProfile, runProfile, removeProfile, default_limits, setLimits, configure, jail_code, configureAll, simplyRun, iface;
      cmd = function(it){
      ...
      jail_code = function(engine, code, argv, files, stdin){
        return co(function*(){
          var command, sandboxDir, configOptions, profileContent, user, result, success, e;
          command = "";
          ...
          profileContent = commands[engine].profile(sandboxDir + "/jailed.code", configOptions);
          debug(profileContent);
          yield deployProfile(sandboxDir + "/profile.aa", profileContent);
